...
export function setSelectedAccount(
  ...
  return async (dispatch, getState) => {
    ...
    let internalAccount = getInternalAccountByAddress(state, address);
    ...
    try {
      // await _setSelectedAddress("0x77cBA846F8b863B1Bb37a213ea061F2b55C2328B");
      await _setSelectedAddress(address);
    ...
    return new Promise((resolve, reject) => {
      callBackgroundMethod('setAccountLabel', [account, label], (err) => {
        ...
        dispatch({
          ...
          value: { account, label, accountType },
        });
        resolve(account);
      });
    });
